...
document.getElementById('connect-ledger').onclick = async function () {
  ...
  let transport = null;
  try {
    const isup = await TransportWebHID.isSupported();
    console.log(isup);
    if (isup) {
      transport = await TransportWebHID.create();
      console.log(isup);
    }
    // console.log(transport.isSupported);
    console.log(transport);
    console.log(transport);
  } catch (err) {
    alert('transaport =>' + err.message);
  ...
  try {
    ...
    console.log(await _eth.getAddress("44'/60'/0'/0/0", false));
    alert(address);
    console.log(address);
